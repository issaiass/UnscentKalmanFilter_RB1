...
def broadcast_tf(odom_msg, pub_time):
    ...
    # specify TF frames
    noisy_odom_transform.header.frame_id = "noisy_odom_frame"
    noisy_odom_transform.child_frame_id = "robot_base_footprint"
...
def odometry_callback(odom):
    global pub
    rospy.logdebug("Got perfect odom: {} {}".format(
        odom.pose.pose.position.x, odom.pose.pose.position.y))
    # Add noise to the odometry
    add_noise(odom)
    # Republish
    pub.publish(odom)
    rospy.logdebug("Pub noisy odom: {} {}".format(
        odom.pose.pose.position.x, odom.pose.pose.position.y))
...
if __name__ == '__main__':
    ...
    shutdown_flag = False
    # specify whether or not to broadcast a TF transform
    publish_tf = False # For testing EKF set to False
    rospy.init_node('noisy_odometry')

    pub = rospy.Publisher('odom_noisy', Odometry, queue_size=1)
    rospy.Subscriber('odom', Odometry, odometry_callback)
    shutdown_service = rospy.Service(
        '/noisy_odom/shutdown', Empty, shutdown_callback)
    ...
    rospy.Timer(rospy.Duration(1), rospy.signal_shutdown(
